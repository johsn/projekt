...
public class Claim {
	...
	public Claim(String ssn, String description, Double costOfDamage,
			Double priceOfCar) {
		this.id = getNextId();
		this.ownerSsn = ssn;
	...
	public void classifyAsSimple() throws AuthenticationException {
		if (CAuthentication.getInstance().getEmployeeRole() == Role.EMPLOYEE) {
			throw new AuthenticationException(
					"employee trying to classify claim");
		}
	...
	public void setCheckHistoryCompleted() throws PriorityException {
		if (this.CheckInsuranceStatus != ClaimStatus.COMPLETED) {
			throw new PriorityException(
					"trying to check history before checking insurance for complex claim");
		}
